...
const log = debug_1.default('sourced');
...
class SourcedEntity extends EventEmitter {
    ...
    emit() {
        if (!this.replaying) {
            events_1.default.EventEmitter.prototype.emit.apply(this, arguments);
        }
    }
    /**
    ...
    digest(method, data) {
        if (!this.replaying) {
            ...
            log(util_1.default.format('digesting event \'%s\' w/ data %j', method, data));
            this.newEvents.push({
                method: method,
                data: data,
     ...
     */
    merge(snapshot) {
        log(util_1.default.format('merging snapshot %j', snapshot));
        for (var property in snapshot) {
            if (Object.prototype.hasOwnProperty.call(snapshot, property)) {
                var val = lodash_clonedeep_1.default(snapshot[property]);
            }
    ...
    mergeProperty(name, value) {
        if (mergeProperties.size &&
            ...
            lodash_merge_1.default(this[name], value);
        else
            this[name] = value;
    }
    /**
    ...
    replay(events) {
        var self = this;
        this.replaying = true;
        log(util_1.default.format('replaying events %j', events));
        events.forEach(function (event) {
            if (self[event.method]) {
                ...
                const errorMessage = util_1.default.format('method \'%s\' does not exist on model \'%s\'', event.method, className.trim());
                log(errorMessage);
    ...
    snapshot() {
        ...
        var snap = lodash_clonedeep_1.default(this, true);
    ...
    static digestMethod(type, fn) {
        ...
        if (!fn)
            throw new EntityError('a function is required for digest method definitions');
        if (!fn.name)
            throw new EntityError('Anonmyous functions are not allowed in digest method definitions. Please provide a function name');
